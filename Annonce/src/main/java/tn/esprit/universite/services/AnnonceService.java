...
import tn.esprit.universite.service.UserServiceClient;
...
public class AnnonceService implements IAnnonceService {
    ...
    public Annonce addAnnonce(Annonce annonce, MultipartFile imageFile) {
        ...
        try {
            User user = userServiceClient.getUserById(annonce.getUserId());
            if (user == null) {
                throw new RuntimeException("User not found with id: " + annonce.getUserId());
            }
        } catch (Exception e) {
